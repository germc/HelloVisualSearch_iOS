...
- (void)addImages
{
    ...
    /// REMOTE
    NSInteger resId = [_myVs insertImageFromURL:[NSURL URLWithString:@"https://s3.amazonaws.com/aumentia/pic_from_url.jpg"]];
    
    if(resId == -1) VSLog(@"Error adding image from URL")
    else            VSLog(@"Image from URL added with id %d", resId);
